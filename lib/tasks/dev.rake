namespace :dev do
  task :seed => :environment do
    ...
    ActiveRecord::Base.transaction do
      Shop.create!({
        server_id: Server.first.id, player: 'Aragorn', pack: false, seen_time: Time.zone.now - 1.hour, loc_x: 1, loc_y: 2, loc_z: 3
      }).products.create([
      ...
      Shop.create!({
        server_id: Server.last.id, player: 'Gimli', pack: false, seen_time: Time.zone.now - 1.day, loc_x: 7, loc_y: 7, loc_z: 7
      }).products.create([
